...
We survyed currently available data standardisation softwares and found that all these efforts are tailored to a specific EHR source, a few generic utilities have been developed such as the graphical-interface-based "White Rabbit", and "Rabbit In A Hat" and purely code-based “Extract, transform, load (ETL) framework”. While the user-interface-centric it comes at the cost of being incompatible with simultaneous collaborative development. Moreover, the SQL code generated by these tools is inefficient. These limitations lead to scalability challenges, replication difficulties, a higher risk of errors, and increased manual effort in reviewing the mappings. On the other hand, so far all the tools including Usagi for standard vocabulary mapping are manual or semi automated at best.

Next, the tools for data quality assurance such as Achilles, DataQualityDashboard (DQD), ARES, MIRACUM, DQA, and mosaicQA of the EHR are passive in nature as they do not provide solution to rectify the detected anomalies. Moreover, they do not handle all types issues that can arise in a typical EHR. For instance, data quality dashboard can only detect anomalies based on pre-set rules. Fruthermore, all the tools are bound to a specific source format. Finally, none of the tools offer solution to correct the detected anomalies.

With this case study, we aim to showcase that EHR-QC can overcome all the shortcomings listed above. To demonstrate that, we have curated a patient cohort sourced from a publicly accessible EHR repository. Employing a range of functionalities offered by EHR-QC, we have processed and structured this data, rendering it suitable for conducting machine learning analysis. This endeavor showcased the simplicity, flexibility, versatility, diverse capabilities, utility, and practicality of the EHR-QC.
